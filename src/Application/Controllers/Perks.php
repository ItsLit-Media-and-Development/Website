...
class Perks extends Library\BaseController
{
    ...
    {
        $emoteData = $this->_guzzle->get('https://api.itslit.uk/twitch/subemotes/itslittany');
        $emoteData = json_decode($emoteData->getBody(), true);
        $emoteData = $emoteData['response'];

        $badgeData = $this->_guzzle->get('https://api.itslit.uk/twitch/subbadges/itslittany');
        $badgeData = json_decode($badgeData->getBody(), true);
        $badgeData = $badgeData['response'];

        $this->_view->setTemplate('Application/View/perks');
